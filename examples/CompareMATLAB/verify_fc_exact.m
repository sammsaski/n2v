...
% Handle multiple stars (exact-star can produce multiple output stars)
if length(R) == 1
    % Single star case (approx-star)
    [lb_out, ub_out] = R.getRanges;
    lb_out = squeeze(lb_out);
    ub_out = squeeze(ub_out);
else
    % Multiple stars case (exact-star)
    % Initialize bounds to extreme values
    lb_out = inf(numClasses, 1);
    ub_out = -inf(numClasses, 1);

    % Compute union of ranges across all stars
    for i = 1:length(R)
        [lb_i, ub_i] = R(i).getRanges;
        lb_i = squeeze(lb_i);
        ub_i = squeeze(ub_i);
        lb_out = min(lb_out, lb_i);
        ub_out = max(ub_out, ub_i);
    end
end
...
% Save the figure
saveas(f, "outputs/exact/matlab_reach_stmnist_plot.png");

%% Save Results for Comparison with Python

% Create output directory if it doesn't exist
if ~exist('outputs/exact', 'dir')
    mkdir('outputs/exact');
end
...
save('outputs/exact/matlab_verify_fc_results.mat', 'results');
fprintf('✅ Results saved to: outputs/exact/matlab_verify_fc_results.mat\n');

% Save detailed text report
fid = fopen('outputs/exact/matlab_verify_fc_results.txt', 'w');
...
fclose(fid);
fprintf('✅ Results saved to: outputs/exact/matlab_verify_fc_results.txt\n');
